...
using GuXin.Core.Tenancy;
using GuXin.Core.Utilities;
using GuXin.Entity;
using GuXin.Entity.DomainModels;
using GuXin.Entity.SystemModels;

namespace GuXin.Core.BaseProvider
{
         ...
         where TRepository : IRepository<T>
    {
        ...
        /// <returns></returns>
        public virtual WebResponseContent Upload(List<Microsoft.AspNetCore.Http.IFormFile> files, string folder = "")
        {
            ...
            string filePath = $"Upload/{(folder.IsNullOrEmpty() ? typeof(T).GetEntityTableName() + "/" + DateTime.Now.ToString("yyyMMddHHmmsss") : folder)}/";
        ...
        public virtual WebResponseContent UpdateEntity(T entity, Expression<Func<T, object>> properties, bool validationEntity = true)
        {
            //设置用户默认值
            ...
            if (!Response.Status) return Response;
            return Response.Info(repository.Update(entity, properties, true));
        }
        /// <summary>
        /// 保存主、明细数据
        /// </summary>
                ...
                foreach (var item in updateFileds)
                {
                    switch (item.Name.ToLower())
                    {
                        ...
                        case "auditstatus":
                            ...
                            break;
                        case "auditor":
                            item.SetValue(entity, userInfo.NickName);
                            break;
                        case "auditdate":
