  ...
  <div class="user-info">
    <div class="left">
      <div>
        ...
        <div class="text">
          ...
          <p>
            <Button style="padding: 3px 16px;" @click="modifyImg" type="info" size="small" ghost>修改头像</Button>
          </p>
        </div>
      </div>
      <Divider>
      ...
      <Card class="binding-group" shadow>
        <CellGroup>
          <Cell :title="'手机：'+(userInfo.phoneNo||'未绑定')">
            <Button type="success" size="small" @click="modifyPhone" icon="md-phone-portrait" slot="extra">修改手机</Button>
          </Cell>
          <Cell :title="'邮箱：'+(userInfo.email||'未绑定')">
            <Button type="info" size="small" @click="modifyEmail" icon="ios-mail-outline" slot="extra">绑定邮箱</Button>
          </Cell>
        </CellGroup>
      </Card>
    ...
    <div class="right">
      <vol-form ref="form" :load-key="true" :width="500" :formRules="editFormOptions" :formFileds="editFormFileds">
        ...
        <div slot="footer">
          <Button style="margin-top: 2px;" type="info" size="large" icon="md-checkmark-circle" long
            @click="modifyInfo">保存</Button>
        </div>
      </vol-form>
    </div>
...
export default {
  ...
  data () {
    return {
      errorImg: 'this.src="' + require("@/assets/imgs/logo.png") + '"',
      modifyOptions: {
        model: false,
