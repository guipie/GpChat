		...
		<view :class="['myp-bg-'+bgType, 'myp-border-'+border, 'myp-nav-box', fixed&&'myp-nav-fixed']" :style="statusNavStyle" @tap.stop="toPrevent">
			...
			<view class="myp-flex-row myp-wrap-nowrap myp-align-center" :style="mrNavStyle">
				<view :class="['myp-flex-row', 'myp-wrap-nowrap', 'myp-justify-start', 'myp-align-center', 'myp-nav-lefts', leftBgType&&('myp-bg-'+leftBgType)]" :style="leftStyle">
					<slot name="left">
						<view v-if="lefts && lefts.length > 0" v-for="(left, idx) in lefts" :key="idx" :class="['myp-flex-row', 'myp-align-center', 'myp-position-relative', left.bgType&&('myp-bg-'+left.bgType)]" :style="leftItemStyle+(left.style||'')" bubble="true" @tap="goLeft(idx)">
				...
				<view :class="['myp-flex-row', 'myp-align-center', 'myp-wrap-nowrap', 'myp-justify-end', 'myp-nav-rights', rightBgType&&('myp-bg-'+rightBgType)]" :style="rightStyle">
					<slot name="right">
						<view v-if="rights && rights.length > 0" v-for="(right, idx) in rights" :key="idx" :class="['myp-flex-row', 'myp-align-center', 'myp-wrap-nowrap', 'myp-justify-end', 'myp-position-relative', right.bgType&&('myp-bg-'+right.bgType)]" :style="rightItemStyle+(right.style||'')" bubble="true" @tap="goRight(idx)">
...
<script>
	import {getStatusBarHeight} from '../mypUI/utils/system.js'
